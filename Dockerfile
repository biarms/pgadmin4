...
FROM ${BUILD_ARCH}node:10-alpine AS app-builder
...
FROM ${BUILD_ARCH}python:3.7-alpine3.10 as docs-builder
...
FROM ${BUILD_ARCH}postgres:9.4-alpine as pg94-builder
FROM ${BUILD_ARCH}postgres:9.5-alpine as pg95-builder
FROM ${BUILD_ARCH}postgres:9.6-alpine as pg96-builder
FROM ${BUILD_ARCH}postgres:10-alpine as pg10-builder
FROM ${BUILD_ARCH}postgres:11-alpine as pg11-builder
FROM ${BUILD_ARCH}postgres:12-alpine as pg12-builder

FROM ${BUILD_ARCH}alpine:3.10 as tool-builder
